        ...
        return (
            <section>
                ...
                <p className="text-center font-medium text-md text-black dark:text-gray-300">
                    Lets <Link className="text-blue-400 dark:text-blue-300 underline underline-offset-1" to="/create">Create One</Link>
                </p>
            </section>
        )
    }
            ...
            {entries.map((entry: Entry, index: number) => {
                return (
                    <div id={entry.id} key={index} className="bg-gray-300 dark:bg-gray-700 shadow-md shadow-gray-500 dark:shadow-gray-900 m-3 p-4 rounded flex flex-col justify-between">
                        ...
                        <p className="text-center text-lg font-light md:mt-2 md:mb-4 mt-1 mb-3 text-black dark:text-gray-300">{entry.description}</p>
                        <section className="flex items-center justify-between flex-col md:flex-row pt-2 md:pt-0">
                            ...
                            <time className="text-right text-sm md:text-lg text-black dark:text-gray-300">{new Date(entry.created_at.toString()).toLocaleDateString()}</time>
                        </section>
                    </div>
                )
            })}
